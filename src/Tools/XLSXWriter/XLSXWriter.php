...
namespace App\Tools\XLSXWriter;

/*
 * Writer simple xlsx
 * https://github.com/mk-j/PHP_XLSXWriter
 * modifié : couper en 2 class / ZipArchive conflit avec phphexcel
 * */
...
class XLSXWriter
{
    ...
    protected function initializeSheet($sheet_name)
    {
        ...
        $tabselected = 1 == count($this->sheets) ? 'true' : 'false'; // only first sheet is selected
    ...
    private function escapeCellFormat($cell_format)
    {
        ...
        for ($i = 0,$ix = strlen($cell_format); $i < $ix; ++$i) {
            ...
            if ('' == $ignore_until && (' ' == $c || '-' == $c || '(' == $c || ')' == $c) && (0 == $i || '_' != $cell_format[$i - 1])) {
                $escaped .= '\\'.$c;
            } else {
                $escaped .= $c;
            }
    ...
    private function addCellFormat($cell_format)
    {
        ...
        if ('string' == $cell_format) {
            $cell_format = 'GENERAL';
        } elseif ('integer' == $cell_format) {
            $cell_format = '0';
        } elseif ('date' == $cell_format) {
            $cell_format = 'DD/MM/YYYY';
        } elseif ('datetime' == $cell_format) {
            $cell_format = 'YYYY-MM-DD HH:MM:SS';
        } elseif ('dollar' == $cell_format) {
            $cell_format = '[$$-1009]#,##0.00;[RED]-[$$-1009]#,##0.00';
        } elseif ('money' == $cell_format) {
            $cell_format = '[$$-1009]#,##0.00;[RED]-[$$-1009]#,##0.00';
        } elseif ('euro' == $cell_format) {
            $cell_format = '#,##0.00 [$€-407];[RED]-#,##0.00 [$€-407]';
        } elseif ('NN' == $cell_format) {
            $cell_format = 'DDD';
        } elseif ('NNN' == $cell_format) {
            $cell_format = 'DDDD';
        } elseif ('NNNN' == $cell_format) {
            $cell_format = 'DDDD", "';
        }
    ...
    protected function writeCell(XLSXWriterBuffererWriter &$file, $row_number, $column_number, $value, $cell_format_index)
    {
        ...
        } elseif ('0' != $value[0] && '+' != $value[0] && filter_var($value, FILTER_VALIDATE_INT, ['options' => ['max_range' => 2147483647]])) {
            $file->write('<c r="'.$cell_name.'" s="'.$cell_format_index.'" t="n"><v>'.($value * 1).'</v></c>');
